 ...
 * @author mwaleria
 */
public class UserCommunicationChanel {
    ...
    public UserCommunicationChanel(ServerUser serverUser, Socket socket, EventDispatcher<ServerRequest> dispatcher, KeyPair serverKeyPair) {
    ...
    private void start(KeyPair serverKeyPair) {
        try {
            inputStreamHandler = new InputStreamHandler(socket.getInputStream(),serverKeyPair.getPrivate(), dispatcher);
    ...
    public void updateUserPublicKey(PublicKey userPublicKey) {
